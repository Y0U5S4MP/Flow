...
const AdvancedPanelEditor: React.FC<Props> = ({ panel, onPanelUpdate, onClose }) => {
  ...
  const addElement = (type: (typeof ELEMENT_BUTTONS)[number]['type']) => {
  ...
  const renderCanvasElement = (el: ComicElement) => {
    ...
    if (el.type === 'text') {
      if (bubbleType) {
        ...
        const bdColor = el.borderColor  || '#222222';
        ...
        inner = isEditing ? (
          <textarea
            ...
            style={{ width: '100%', height: '100%', border: '1px dashed #8b5cf6', background: 'rgba(255,255,255,0.8)',
                     ...
                     color: el.color || '#1a1a1a', fontWeight: el.fontWeight || 'normal',
                     fontStyle: el.fontStyle || 'normal', fontFamily: el.fontFamily || 'Arial',
      ...
      inner = <img src={el.imageUrl} alt="" style={{ width: '100%', height: '100%', objectFit: el.objectFit || 'contain', display: 'block' }} />;
    } else if (el.type === 'gif') {
          ...
          <div className="w-72 border-l border-gray-200 bg-white flex flex-col overflow-y-auto flex-shrink-0">
            <div className="px-4 py-3 border-b border-gray-100 bg-gray-50">
              <p className="text-sm font-bold text-gray-700 flex items-center gap-2">
                ...
                {selected
                  ? (selected.bubbleType === 'speech' ? 'Globo de diálogo' :
                     selected.bubbleType === 'thought' ? 'Burbuja de pensamiento' :
                     selected.bubbleType === 'caption' ? 'Caja de narración' :
              ...
              {!selected ? (
                  ...
                  <Section id="orderEditor" title="¿Cuándo aparece cada elemento?" icon={Timer}>
                    ...
                    <div className="space-y-1">
                      {sortedEls.map((el, idx) => {
                        ...
                        const elLabel =
                          el.bubbleType === 'speech' ? 'Globo de diálogo' :
                          el.bubbleType === 'thought' ? 'Pensamiento' :
                          el.bubbleType === 'caption' ? 'Narración' :
                ...
                <div className="space-y-3">
                  ...
                  <Section id="position" title="Posición y Tamaño" icon={Move}>
                    <div className="grid grid-cols-2 gap-2">
                      {([['X', 'x', 0, panelW], ['Y', 'y', 0, panelH], ['Ancho', 'width', 10, panelW], ['Alto', 'height', 10, panelH]] as [string, 'x' | 'y' | 'width' | 'height', number, number][]).map(([label, key, min, max]) => (
                        <div key={key}>
                          <label className="block text-xs text-gray-500 mb-1">{label}</label>
                          <input type="number" min={min} max={max}
                            value={Math.round(selected[key] || 0)}
                            onChange={e => updateEl(selected.id, { [key]: +e.target.value } as Partial<ComicElement>)}
                  ...
                  {selected.type === 'text' && (
                    <Section id="textStyle" title="Texto y Estilo" icon={Type}>
                      ...
                      <div>
                        ...
                        <select value={selected.fontFamily || 'Arial'} onChange={e => updateEl(selected.id, { fontFamily: e.target.value })}
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        ...
                        {selected.bubbleType && <div><label className="block text-xs text-gray-500 mb-1">Color fondo</label><input type="color" value={selected.bgColor || '#ffffff'} onChange={e => updateEl(selected.id, { bgColor: e.target.value })} className="w-full h-9 border border-gray-200 rounded-lg cursor-pointer" /></div>}
                      </div>
                      <div className="flex gap-2">
                        ...
                        ] as { label: string; prop: 'fontWeight' | 'fontStyle' | 'textDecoration'; on: string; off: string; style: string }[]).map(({ label, prop, on, off, style }) => (
                          <button key={prop} onClick={() => updateEl(selected.id, { [prop]: selected[prop] === on ? off : on } as Partial<ComicElement>)}
                            className={`flex-1 py-2 rounded-lg text-sm border transition-all ${selected[prop] === on ? 'bg-purple-600 text-white border-purple-600' : 'bg-gray-50 border-gray-200 text-gray-700 hover:bg-gray-100'} ${style}`}>
                            {label}
                          </button>
                        ))}
                      </div>
                  ...
                  {(selected.type === 'image' || selected.type === 'gif') && (
                    <Section id="imgStyle" title="Imagen" icon={ImageIcon}>
                      ...
                      <div><label className="block text-xs text-gray-500 mb-1">Ajuste</label>
                        <select value={selected.objectFit || 'contain'} onChange={e => updateEl(selected.id, { objectFit: e.target.value as ComicElement['objectFit'] })}
                          className="w-full border border-gray-200 rounded-lg px-2 py-1.5 text-sm bg-white">
                  ...
                  {selected.type === 'video' && (
                    <Section id="videoStyle" title="Video" icon={Film}>
                      ...
                      <Slider label="Opacidad" value={(selected.opacity || 1) * 100} min={10} max={100} unit="%" onChange={v => updateEl(selected.id, { opacity: v / 100 })} />
                      <div><label className="block text-xs text-gray-500 mb-1">Ajuste</label>
                        <select value={selected.objectFit || 'cover'} onChange={e => updateEl(selected.id, { objectFit: e.target.value as ComicElement['objectFit'] })}
                          className="w-full border border-gray-200 rounded-lg px-2 py-1.5 text-sm bg-white">
                          <option value="cover">Rellenar (Cover)</option>
                          <option value="contain">Ajustar (Contain)</option>
                          <option value="fill">Estirar (Fill)</option>
                  ...
                  <Section id="anim" title="¿Cómo aparece este elemento?" icon={Play}>
                    ...
                    <select value={selected.entranceAnimation?.type || ''}
                      onChange={e => { const t = e.target.value as ElementAnimation['type'] | ''; updateEl(selected.id, { entranceAnimation: t ? { type: t, duration: 500, delay: 0 } : undefined }); }}
                      className="w-full border border-gray-200 rounded-lg px-2 py-1.5 text-sm bg-white">
                      ...
                      <>
                        {selected.entranceAnimation.type === 'slideIn' && (
                          <div><label className="block text-xs text-gray-500 mb-1">¿Desde dónde entra?</label>
                            <select value={selected.entranceAnimation.direction || 'left'} onChange={e => updateEl(selected.id, { entranceAnimation: { ...selected.entranceAnimation!, direction: e.target.value as NonNullable<ElementAnimation['direction']> } })}
                              className="w-full border border-gray-200 rounded-lg px-2 py-1.5 text-sm bg-white">
